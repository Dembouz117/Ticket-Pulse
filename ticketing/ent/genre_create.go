...
func (gc *GenreCreate) AddConcertIDs(ids ...uuid.UUID) *GenreCreate {
	gc.mutation.AddConcertIDs(ids...)
	return gc
}

// AddConcerts adds the "concerts" edges to the Concert entity.
func (gc *GenreCreate) AddConcerts(c ...*Concert) *GenreCreate {
	ids := make([]uuid.UUID, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return gc.AddConcertIDs(ids...)
}
...
func (gc *GenreCreate) createSpec() (*Genre, *sqlgraph.CreateSpec) {
	...
	if nodes := gc.mutation.ConcertsIDs(); len(nodes) > 0 {
