...
type TicketQuery struct {
	...
	withWithinSection *SectionQuery
	withFKs           bool
...
// QueryWithinSection chains the current query on the "withinSection" edge.
func (tq *TicketQuery) QueryWithinSection() *SectionQuery {
	query := (&SectionClient{config: tq.config}).Query()
...
func (tq *TicketQuery) Clone() *TicketQuery {
	...
	return &TicketQuery{
		...
		withWithinSection: tq.withWithinSection.Clone(),
...
// WithWithinSection tells the query-builder to eager-load the nodes that are connected to
// the "withinSection" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TicketQuery) WithWithinSection(opts ...func(*SectionQuery)) *TicketQuery {
	query := (&SectionClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withWithinSection = query
	return tq
}
...
//
//	client.Ticket.Query().
//		GroupBy(ticket.FieldSeatNumber).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (tq *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [1]bool{
			tq.withWithinSection != nil,
		}
	)
	if tq.withWithinSection != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withWithinSection; query != nil {
		if err := tq.loadWithinSection(ctx, query, nodes, nil,
			func(n *Ticket, e *Section) { n.Edges.WithinSection = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TicketQuery) loadWithinSection(ctx context.Context, query *SectionQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Section)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ticket)
	for i := range nodes {
		if nodes[i].section_has_tickets == nil {
			continue
		}
		fk := *nodes[i].section_has_tickets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "section_has_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
