...
// QueryConcerts chains the current query on the "concerts" edge.
func (gq *GenreQuery) QueryConcerts() *ConcertQuery {
	query := (&ConcertClient{config: gq.config}).Query()
...
func (gq *GenreQuery) Clone() *GenreQuery {
	...
	return &GenreQuery{
		...
		withConcerts: gq.withConcerts.Clone(),
...
// WithConcerts tells the query-builder to eager-load the nodes that are connected to
// the "concerts" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GenreQuery) WithConcerts(opts ...func(*ConcertQuery)) *GenreQuery {
	query := (&ConcertClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withConcerts = query
	return gq
}
...
func (gq *GenreQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Genre, error) {
	...
	if query := gq.withConcerts; query != nil {
		if err := gq.loadConcerts(ctx, query, nodes,
			func(n *Genre) { n.Edges.Concerts = []*Concert{} },
			func(n *Genre, e *Concert) { n.Edges.Concerts = append(n.Edges.Concerts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GenreQuery) loadConcerts(ctx context.Context, query *ConcertQuery, nodes []*Genre, init func(*Genre), assign func(*Genre, *Concert)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(genre.ConcertsTable)
		s.Join(joinT).On(s.C(concert.FieldID), joinT.C(genre.ConcertsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(genre.ConcertsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(genre.ConcertsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "concerts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
