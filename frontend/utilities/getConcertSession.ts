...
export const getConcertSession = async (artist:string): Promise<string> => {
    try {
        //gets concerts, then sessions
        const concertSession = await axios.get(`${getTicketingApiUrl()}/api/v1/concerts/artist/${artist}`)
        .then(res=>{
